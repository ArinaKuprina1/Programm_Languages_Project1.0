...
class RTError(Error):
    ...
    # Generates a traceback for the error, showing the call stack.
...
class Position:
    ...
    # Updates line and column numbers if a newline is encountered.
    # Returns self.
    ...
    # Creates and returns a copy of the current Position object.
    def copy(self):
...
class Lexer:
    ...
    # Tokenizes the input text, converting it into a list of Token objects.
    # Returns a tuple of (tokens, error).
    ...
    # Creates a Token for either a minus sign or an arrow (->).
    ...
    # Creates a Token for an integer or float number.
    # Returns a Token object.
    ...
    # Creates a Token for an identifier or keyword.
    # Returns a Token object.
    def make_identifier(self):
        ...
        if id_str == 'Lambd':
            return Token(TT_LAMBD, id_str, pos_start, self.pos)
        else:
            tok_type = TT_KEYWORD if id_str in KEYWORDS else TT_IDENTIFIER
            return Token(tok_type, id_str, pos_start, self.pos)

    # Creates a Token for the not-equals operator (!=).
    # Returns a tuple of (Token, error).
    ...
    # Creates a Token for the equals (=) or double-equals (==) operator.
    # Returns a Token object.
    ...
    # Creates a Token for the less-than (<) or less-than-or-equal (<=) operator.
    # Returns a Token object.
    ...
    # Creates a Token for the greater-than (>) or greater-than-or-equal (>=) operator.
    # Returns a Token object.
...
class LambdaNode:
    def __init__(self, param_name_tok, body_node):
        self.param_name_tok = param_name_tok
        self.body_node = body_node

        self.pos_start = self.param_name_tok.pos_start
        self.pos_end = self.body_node.pos_end
...
class Parser:
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'VAR', 'IF', 'FUN', int, float, identifier, '+', '-', '(' or 'NOT'"
            ))
    ...
    def lambda_expr(self):
        ...
        if not self.current_tok.type == TT_LAMBD:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'Lambd'"
            ))
        ...
        return res.success(LambdaNode(param_name, body))
...
class Interpreter:
    ...
    def visit_CallNode(self, node, context):
        ...
        elif isinstance(value_to_call, LambdaNode):
            # Handle lambda expressions
            lambda_func = res.register(self.visit(value_to_call, context))
            if res.error: return res
            value_to_call = lambda_func.copy().set_pos(node.pos_start, node.pos_end)
        else:
            return res.failure(RTError(
                node.pos_start, node.pos_end,
    ...
    def visit_LambdaNode(self, node, context):
        ...
        param_name = node.param_name_tok
        body = node.body_node

        function = Function(None, body, [param_name.value]).set_context(context).set_pos(node.pos_start, node.pos_end)
